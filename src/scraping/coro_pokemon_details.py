...
async def get_pokemon_details(
...
) -> Coroutine[Any, Any, Awaitable[Dict[str, list]]]:
    # Storage
    storage = {
        "name": list(),
        "description": list(),
    ...
    # Create batches for concurrent tasks
    batches = await utils.batch_iterable(
        [card["url"] for card in data_pokedex_cards_img], 5
    )
    ...
    for batch in batches:
        for url in batch:
            tasks = [
                scrape(browser=browser, url_pokemon=url, db=storage)
            ]
...
async def scrape(
    browser: Browser, url_pokemon: str, db: Dict[str, list]
):
    # Create new page
